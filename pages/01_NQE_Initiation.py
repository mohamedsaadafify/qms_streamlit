...
with c1: st.text_input("NQE# (auto)", value="Will be assigned on submit", disabled=True)
with c2: st.text_input("Date/Time stamp", value=datetime.now().strftime("%d/%m/%Y – %H:%M"), disabled=True)
with c3: st.text_input("Created by", value=username, disabled=True)
...
if st.button("Submit NQE", type="primary"):
    with get_conn() as con:
        ...
        """, (
            next_seq("NQE"), datetime.now().isoformat(), username, product, batch_no, deviated_code,
            occurrence, discovery, initiated_dept, what, why_dev, who, where, how, containment, corrections, mitigate, "submitted"
        ))
        con.commit()
